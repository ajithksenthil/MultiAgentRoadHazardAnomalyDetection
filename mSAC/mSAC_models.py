...
class Actor(nn.Module):
    ...
    def forward(self, state, hx):
        ...
        if state.dim() == 3:
            ...
            state = state.view(-1, state.size(-1)) # removal does not fix error
...
class Critic(nn.Module):
    ...
    def forward(self, state, action, hx):
        batch_size = state.size(0)  # Get the batch size from the state tensor
        hx_clone = hx.clone()
        hx_clone = hx_clone.expand(batch_size, -1).contiguous()
...
class MixingNetwork(nn.Module):
    ...
    def forward(self, states, local_qs):
        ...
        # print("local qs size: ", local_qs.size()) # this is [128, 5]
